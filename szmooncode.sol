...
contract SZMoon is Context, IERC20, Ownable {
    ...
    modifier checkIfStart(address from,address to ,uint256 amount) {
        if(from != owner() && to != owner())
        {
            ...
            require(!(block.number <= startBlock + presalePeriodBlocks && amount> _presalePeriodTxAmountLimit), "tx amount is limit to 10,000 on presale period");
        }
        _;
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOwner {
        require(_token != IERC20(this), "This function is only used for token transferred in by mistake");
